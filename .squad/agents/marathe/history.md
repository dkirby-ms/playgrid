...
- 2026-03-16: PR #123 (`dev` → `uat`) conflicted in `client/src/renderers/BackgammonRenderer.ts`, `client/src/renderers/CheckersRenderer.ts`, `client/src/scenes/GameScene.ts`, and `client/src/ui/GameSidebar.ts`. After inspecting the conflict markers, resolution favored `origin/dev` for the redesigned renderer/sidebar implementations while preserving the additive pattern: Backgammon kept manual roll controls, Checkers kept head-to-head-aware turn styling and perspective logic, GameScene kept the newer turn-prompt cleanup reset, and the add/add sidebar conflict reduced to taking dev’s extra active-turn CSS blocks because UAT only had the earlier base file.
